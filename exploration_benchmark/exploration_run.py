...
def run_inference_and_evaluation(args, datapoints, rules):
    ...
    for i, datapoint in enumerate(tqdm(datapoints)):
        ...
        print(f'Compatibility: {compatibility}, Is_minimum_description: {is_minimum_description}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--k', type=int, default=4, help='number of k in IOSL')
    ...
    parser.add_argument('--sample_size', type=int, default=100, help='number of sample to compute induction on, 1 means it is just the characteristic sample')
    ...
    for k in range(2, args.k+1): # k = 2, 3, 4
        for vocab_size in range(5, args.vocab_size+1): # vocab_size = 5, 6, 7, 8
            for number_of_rules in range(3, args.number_of_rules+1): # number_of_rules = 3, 4, 5
                ...
                average_compatibility, average_is_minimum_description = run_inference_and_evaluation(args, datapoints, rules)
